...
namespace miniconf
{
    ...
    public:
        ...
        inline explicit Value(const int& other);

        // Constructs a Value instance from a floating point
        inline explicit Value(const double& other);

        // Constructs a Value instance from a boolean
        inline explicit Value(const bool& other);

        // Constructs a Value instance from a char array
        inline explicit Value(const char* other);

        // Constructs a Value instance from a std::string
        inline explicit Value(const std::string& other);

        // Assigns an integer to a Value instance
        inline Value& operator=(const int& other);

        // Assigns a floating point to a Value instance
        inline Value& operator=(const double& other);

        // Assigns a boolean to a Value instance
        inline Value& operator=(const bool& other);

        // Assigns a char array to a Value instance
        inline Value& operator=(const char* other);

        // Assigns a std::string to a Value instance
        inline Value& operator=(const std::string& other);
        ...
        inline char* getCharArray() const;
        ...
        inline static Value unknown();

    private:

        // Moves value data from a pointer
        inline Value& moveData(char*& src, const size_t size, const DataType type);

        // Copies value data from a pointer
        inline Value& copyData(const char* src, const size_t size, const DataType& type);
    ...
    public:
        ...
        inline Config::Option& option(const std::string& flag);

        // Removes an option
        inline bool remove(const std::string& flag);

        // Checks if the option value is defined in the current configuration
        inline bool contains(const std::string& flag);
        ...
        inline void print(FILE* fd = stdout);

        // Prints the current log messages
        inline void log(FILE* fd = stdout);

        // Sets the logging level, NO
        inline void log(const LogLevel logType);

        // display the log message
        inline void verbose(bool value);
         ...
         */
        inline void config(const std::string& configPath);
         ...
         */
#ifdef MINICONF_JSON_SUPPORT
        inline std::string serialize(const std::string& serializeFilePath = "", ExportFormat format = ExportFormat::JSON, bool pretty = true);
#else
    ...
    private:
        ...
        inline TokenType getTokenType(const char* token);

        // since long flag is the key for the option directory,
        // this function transltes short flag to long flag
        inline std::string translateShortflag(const std::string& shortflag);

        // search for options using token
        inline Option* getOption(const char* token, Config::TokenType tokenType);

        // determine is a flag is defined in the config
        inline bool findOption(const std::string& flag);

        // parse a token into Value
        inline Value parseValue(const char* token, Value::DataType dataType);

#ifdef MINICONF_JSON_SUPPORT
        // load json config string
        inline bool loadJSON(const std::string& JSONStr);

        // parse a json value
        inline bool parseJSON(const picojson::value *v, const std::string& flag);

        // load a value from json
        inline bool getJSONValue(const picojson::value *v, const std::string& flag);
#endif

        // load csv config string
        inline bool loadCSV(const std::string& CSVStr);

        // internal function for adding log messages
        inline void log(LogLevel logType, const std::string& token, const std::string& msg);
